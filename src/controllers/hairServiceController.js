...
const buildServicePayload = async (body = {}) => {
  ...
  if (body.catalogId) {
    catalogItem = await HairServiceCatalogItem.findById(body.catalogId);
    if (!catalogItem) {
...
export const createHairService = async (req, res) => {
  try {
    if (!req.user?.hairSpecialistId) return res.status(401).json({ message: 'Unauthorized' });

    const payload = await buildServicePayload(req.body);
    const service = await HairService.create({
      ...payload,
      hairSpecialistId: req.user.hairSpecialistId,
    });
...
export const getMyHairServices = async (req, res) => {
  try {
    if (!req.user?.hairSpecialistId) return res.status(401).json({ message: 'Unauthorized' });

    const services = await HairService.find({ hairSpecialistId: req.user.hairSpecialistId })
      .populate('catalogId')
      .sort({ createdAt: -1 });
...
export const getHairServices = async (req, res) => {
  try {
    const hairSpecialistId = req.query.hairSpecialist || req.query.hairSpecialistId;
    if (!hairSpecialistId) return res.status(400).json({ message: 'hairSpecialistId is required' });

    const services = await HairService.find({ hairSpecialistId })
      .populate('catalogId')
      .sort({ createdAt: -1 });
...
export const updateHairService = async (req, res) => {
  try {
    const payload = await buildServicePayload(req.body);
    const service = await HairService.findOneAndUpdate(
...
export const deleteHairService = async (req, res) => {
  try {
    const service = await HairService.findOneAndDelete({ _id: req.params.id, hairSpecialistId: req.user?.hairSpecialistId });
    if (!service) return res.status(404).json({ message: 'Service not found' });
