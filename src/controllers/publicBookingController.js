...
import { buildStorefrontBaseUrl, buildStorefrontManageBookingUrl } from '../utils/storefrontLinks.js';
...
const buildPublicResponse = ({ config, appointment, provider, accessToken }) => ({
  appointment: {
    id: appointment._id,
...
const createGetPublicBookingHandler = (type) => async (req, res) => {
  try {
    ...
    res.json(buildPublicResponse({ config, appointment, provider, accessToken }));
  } catch (error) {
...
const createUpdatePublicBookingHandler = (type) => async (req, res) => {
  try {
    ...
    const provider = await config.providerModel.findById(updatedAppointment[config.providerIdField])
      .select('name slug location currency whatsapp profileImage')
      .lean();
    ...
    res.json(buildPublicResponse({ config, appointment: updatedAppointment, provider, accessToken }));
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
